...
public:
	...
	PixelIndices(Size cellRow, Size cellCol, Size scale);

	/// <summary>
	/// 算出した全てのピクセルに処理を適用する
	/// </summary>
	/// <param name="func">適用する処理</param>
	void ApplyToAll(std::function<void(Size&)> func);

private:
	std::vector<Size> indices;
	Size scale;

	void Create(Size cellRow, Size cellCol);
	void AddIndices(Size pixelRow, Size startCol);
	void RepaetScaleTimes(std::function<void(Size)> func) const;
};
